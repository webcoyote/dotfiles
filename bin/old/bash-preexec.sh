...
__bp_preexec_invoke_exec() {
    ...
    # patw - changed "sed" to "/usr/bin/sed" to work with direnv problems on Windows
    local this_command
    this_command=$(
        export LC_ALL=C
        HISTTIMEFORMAT= builtin history 1 | /usr/bin/sed '1 s/^ *[0-9][0-9]*[* ] //'
    )
...
__bp_install() {
    ...
    # patw - changed "sed" to "/usr/bin/sed" to work with direnv problems on Windows
    local prior_trap=$(/usr/bin/sed "s/[^']*'\(.*\)'[^']*/\1/" <<<"${__bp_trap_string:-}")
